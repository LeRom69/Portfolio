...
export default function Lightbox({ images, startIndex, onClose }) {
    ...
    const onTouchMoveRaw = useCallback((e) => {
        if (e.touches.length === 2 && touchPinchDist.current) {
            e.preventDefault();
            const dist = getDist(e.touches);
            const ratio = dist / touchPinchDist.current;
            setZoom(Math.min(5, Math.max(1, touchZoomStart.current * ratio)));
            return;
        }
        if (e.touches.length === 1) {
            const t = e.touches[0];
            if (zoom > 1 && isDragging.current) {
                e.preventDefault();
                setPan({
    ...
    useEffect(() => {
        ...
        return () => el.removeEventListener("touchmove", onTouchMoveRaw);
    }, [onTouchMoveRaw]);
        ...
        <div className="lb-overlay" onClick={onClose}>
            ...
            <div>
                <div className="lb-zoom-controls" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button onClick={() => setZoom((z) => Math.min(5, z + 0.5))}>+</button>
